...
public class BoardingServiceImpl implements BoardingService {
    ...
    public BoardingBookingResponse createBooking(BoardingBookingCreateRequest request) {
        ...
        BigDecimal estimatedPrice = calculateAmount(
                request.expectedCheckinAt(),
                request.expectedCheckoutAt(),
                roomType.getBaseDailyPriceVnd());
    ...
    private BigDecimal calculateAmount(OffsetDateTime startAt, OffsetDateTime endAt, BigDecimal unitPrice) {
        long billableDays = calculateBillableDays(startAt, endAt);
        return BigDecimal.valueOf(billableDays).multiply(unitPrice).setScale(2, RoundingMode.HALF_UP);
    }
