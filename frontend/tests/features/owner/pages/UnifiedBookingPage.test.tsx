...
describe("UnifiedBookingPage", () => {
  beforeEach(() => {
    ...
    vi.mocked(petApi.getPets).mockResolvedValue({
      content: [{ id: "pet-1", name: "Milu", ownerId: "owner-1", speciesId: "dog", breedId: "poodle", sex: "MALE", birthDate: "", estimatedAgeMonths: 12, weightKg: 4, color: "", identificationNote: "", specialNote: "", allergyNote: "", nutritionNote: "", isActive: true, healthAlerts: [] }],
      pageNumber: 1,
      pageSize: 10,
      totalElements: 1,
      totalPages: 1,
  ...
  it("should_submit_booking_when_form_is_valid", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Ngày nhận phòng"), { target: { value: "2026-06-10" } });
    fireEvent.change(screen.getByLabelText("Giờ nhận phòng"), { target: { value: "09:00" } });
    fireEvent.change(screen.getByLabelText("Ngày trả phòng"), { target: { value: "2026-06-12" } });
    ...
    fireEvent.change(screen.getByLabelText("Yêu cầu chăm sóc đặc biệt"), { target: { value: "Cho ăn hạt riêng" } });
    await userEvent.click(screen.getByRole("button", { name: /Gửi yêu cầu đặt phòng/i }));

    await waitFor(() => {
      expect(boardingApi.createBooking).toHaveBeenCalledWith(expect.objectContaining({
        petId: "pet-1",
        roomTypeId: "type-1",
