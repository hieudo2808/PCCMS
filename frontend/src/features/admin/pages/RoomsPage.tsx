...
export function RoomsPage() {
    ...
    const roomTypesQuery = useQuery({
        queryKey: ["room-types"],
        queryFn: () => roomAdminApi.getRoomTypes(),
    });
    ...
    const createRoomTypeMutation = useMutation({
        mutationFn: (values: RoomTypeFormValues) =>
            roomAdminApi.createRoomType(values as RoomTypeRequest),
        onSuccess: () => {
    ...
    const updateStatusMutation = useMutation({
        mutationFn: ({ id, statusCode }: { id: string; statusCode: RoomStatus }) =>
            roomAdminApi.updateRoomStatus(id, statusCode),
        ...
        <div className="space-y-6">
            <div className="grid gap-6 xl:grid-cols-2">
                <Card
                    ...
                    subtitle="Giá/ngày sẽ được dùng để tính tạm tính và hóa đơn checkout."
                >
                    <form
                        className="space-y-4"
                        onSubmit={roomTypeForm.handleSubmit((values) =>
                    ...
                    >
                        <div className="grid gap-4 md:grid-cols-2">
                            <Input
                                label="Mã loại phòng"
                                error={roomTypeForm.formState.errors.code?.message}
                                {...roomTypeForm.register("code")}
                            />
                            <Input
                                label="Tên loại phòng"
                                error={roomTypeForm.formState.errors.name?.message}
                                {...roomTypeForm.register("name")}
                            />
                            <Input
                                type="number"
                                label="Suc chua mac dinh"
                                error={roomTypeForm.formState.errors.defaultCapacity?.message}
                                {...roomTypeForm.register("defaultCapacity", {
                                    valueAsNumber: true,
                                })}
                            />
                            <Input
                                type="number"
                                label="Giá/ngày"
                                error={roomTypeForm.formState.errors.baseDailyPriceVnd?.message}
                                {...roomTypeForm.register("baseDailyPriceVnd", {
                                    valueAsNumber: true,
                                })}
                            />
                        </div>
                ...
                <Card
                    title="Thêm phòng"
                    subtitle="Phòng cụ thể sẽ được nhân viên gán khi duyệt booking."
                >
                    <form
                        className="space-y-4"
                    ...
                    >
                        <div className="grid gap-4 md:grid-cols-2">
                            <Input
                                label="Mã phòng"
                                error={roomForm.formState.errors.roomCode?.message}
                                {...roomForm.register("roomCode")}
                            />
                            <Input
                                label="Tên phòng"
            ...
            <Card title="Danh sách loại phòng">
                {roomTypesQuery.isLoading ? (
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                        <Loader2 className="h-4 w-4 animate-spin" /> Đang tải loại phòng
                    </div>
                ) : roomTypes.length === 0 ? (
                    <EmptyState
                        title="Chưa có loại phòng"
                        description="Tạo loại phòng trước khi thêm phòng."
                    />
                ) : (
                        ...
                        {roomTypes.map((type) => (
                            <div key={type.id} className="rounded-2xl border border-slate-200 p-4">
                                ...
                                <p className="mt-3 text-sm font-semibold text-emerald-700">
                                    {formatCurrency(type.baseDailyPriceVnd)}/ngày
                                </p>
                            </div>
                        ))}
                    </div>
            ...
            <Card title="Kho phòng lưu trú">
                {roomsQuery.isLoading ? (
                    ...
                    <EmptyState
                        title="Chưa có phòng"
                        description="Tạo phòng để nhận booking lưu trú."
                    />
                ) : (
                        ...
                        {rooms.map((room) => (
                            <div key={room.id} className="rounded-2xl border border-slate-200 p-4">
                                ...
                                <div className="mt-4 flex flex-wrap gap-2">
                                    {(["AVAILABLE", "MAINTENANCE", "INACTIVE"] as RoomStatus[]).map(
                                        (statusCode) => (
                                            <Button
                                                key={statusCode}
                                                variant={
                                                    room.statusCode === statusCode
                                                        ? "secondary"
                                                        : "outline"
                                                }
