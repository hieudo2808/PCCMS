...
export function GroomingAdminPanel() {
  ...
  if (servicesQuery.isError || stationsQuery.isError) {
    return <EmptyState title="Không thể tải danh mục làm đẹp" description="Vui lòng thử lại sau." />;
  }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      <Card title="Dịch vụ làm đẹp">
        <form className="mb-5 grid gap-3 md:grid-cols-2" onSubmit={serviceForm.handleSubmit((data) => createServiceMutation.mutate(data))}>
          ...
          <Input label="Giá VND" type="number" {...serviceForm.register("basePriceVnd")} error={serviceForm.formState.errors.basePriceVnd?.message} />
          <Input label="Thời lượng phút" type="number" {...serviceForm.register("durationMinutes")} error={serviceForm.formState.errors.durationMinutes?.message} />
          <Input label="Mô tả" className="md:col-span-2" {...serviceForm.register("description")} error={serviceForm.formState.errors.description?.message} />
          <Button type="submit" disabled={createServiceMutation.isPending} className="md:col-span-2">
            {createServiceMutation.isPending ? "Đang lưu..." : "Thêm dịch vụ làm đẹp"}
          </Button>
        </form>
        {serviceRows.length === 0 ? (
          <EmptyState title="Chưa có dịch vụ làm đẹp" description="Thêm dịch vụ để chủ nuôi có thể đặt lịch." />
        ) : (
          <DataTable columns={["Mã", "Tên", "Thời lượng", "Giá", "Trạng thái", "Hành động"]} rows={serviceRows} />
        )}
      </Card>

      <Card title="Khu làm đẹp">
        <form className="mb-5 grid gap-3 md:grid-cols-2" onSubmit={stationForm.handleSubmit((data) => createStationMutation.mutate(data))}>
          <Input label="Mã station" {...stationForm.register("stationCode")} error={stationForm.formState.errors.stationCode?.message} />
          <Input label="Tên station" {...stationForm.register("name")} error={stationForm.formState.errors.name?.message} />
        ...
        {stationRows.length === 0 ? (
          <EmptyState title="Chưa có station" description="Thêm station để nhân viên có thể gán lịch làm đẹp." />
        ) : (
          <DataTable columns={["Mã", "Tên", "Trạng thái", "Hành động"]} rows={stationRows} />
        )}
      </Card>
    </div>
  );
