...
export function GroomingTrackingPage() {
    ...
    const cancelMutation = useMutation({
        mutationFn: (ticketId: string) =>
            groomingApi.cancelTicket(ticketId, "Chủ nuôi hủy lịch trước khi nhân viên duyệt"),
        onSuccess: () => {
    ...
    if (ticketsQuery.isError) {
        return (
            <EmptyState title="Không thể tải lịch làm đẹp" description="Vui lòng thử lại sau." />
        );
    }
    ...
    if (tickets.length === 0) {
        return (
            <EmptyState
                ...
                description="Các yêu cầu làm đẹp của bạn sẽ hiển thị tại đây."
            />
        );
    }
        ...
        <div className="space-y-4">
            {tickets.map((ticket) => (
                <Card
                    ...
                    right={
                        <Tag tone={statusTones[ticket.statusCode]}>
                            {statusLabels[ticket.statusCode]}
                        </Tag>
                    }
                >
                    <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <SummaryRow
                                label="Khu làm đẹp"
                                value={ticket.stationName ?? "Nhân viên chưa sắp xếp"}
                            />
                            <SummaryRow
                                label="Nhân viên"
                                value={ticket.assignedStaffName ?? "Chưa phân công"}
                            />
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            {ticket.invoice ? (
                                <div className="space-y-2">
                                    <SummaryRow
                                        label="Mã hóa đơn"
                                        value={ticket.invoice.invoiceCode}
                                    />
                                    <SummaryRow
                                        label="Trạng thái"
                                        value={
                                            ticket.invoice.statusCode === "UNPAID"
                                                ? "Chờ thanh toán"
                                                : ticket.invoice.statusCode
                                        }
                                    />
                                    <SummaryRow
                                        label="Tổng tiền"
                                        value={formatCurrency(ticket.invoice.totalAmountVnd)}
                                    />
                                </div>
                            ) : (
                                <p className="text-sm text-slate-500">
                                    Hóa đơn sẽ xuất hiện sau khi dịch vụ hoàn thành.
                                </p>
                            )}
                        </div>
                    </div>
