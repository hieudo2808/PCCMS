...
const bookingSchema = z
  ...
  .refine((data) => {
    const checkinAt = combineLocalDateTime(data.expectedCheckinDate, data.expectedCheckinTime);
    const checkoutAt = combineLocalDateTime(data.expectedCheckoutDate, data.expectedCheckoutTime);
    return Boolean(checkinAt && checkoutAt && new Date(checkoutAt).getTime() > new Date(checkinAt).getTime());
  }, {
...
export function UnifiedBookingPage() {
  ...
  const checkinAt = useMemo(() => combineLocalDateTime(checkinDate, checkinTime), [checkinDate, checkinTime]);
  const checkoutAt = useMemo(() => combineLocalDateTime(checkoutDate, checkoutTime), [checkoutDate, checkoutTime]);
  const selectedRoomTypeId = watch("roomTypeId");
  const canCheckAvailability = Boolean(checkinAt && checkoutAt && new Date(checkoutAt).getTime() > new Date(checkinAt).getTime());
  ...
  const availabilityQuery = useQuery({
    ...
    queryFn: () => boardingApi.getAvailability(toApiDateTime(checkinDate, checkinTime), toApiDateTime(checkoutDate, checkoutTime)),
  ...
  const createBookingMutation = useMutation({
    mutationFn: (values: BookingFormValues) =>
      boardingApi.createBooking({
        ...
        expectedCheckinAt: toApiDateTime(values.expectedCheckinDate, values.expectedCheckinTime),
        expectedCheckoutAt: toApiDateTime(values.expectedCheckoutDate, values.expectedCheckoutTime),
  ...
  const selectedAvailability = availabilityQuery.data?.find((item) => item.roomTypeId === selectedRoomTypeId);
  const billableDays = useMemo(() => calculateBillableDays(checkinAt, checkoutAt), [checkinAt, checkoutAt]);
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
      ...
      >
        {petsQuery.isLoading ? (
          ...
          <EmptyState title="Không thể tải hồ sơ thú cưng" description="Vui lòng thử lại sau." />
        ) : petOptions.length === 0 ? (
          <EmptyState title="Chưa có thú cưng" description="Hãy tạo hồ sơ thú cưng trước khi đặt phòng." />
        ) : (
          <form className="space-y-5" onSubmit={handleSubmit((values) => createBookingMutation.mutate(values))}>
            <div className="grid gap-4 md:grid-cols-2">
              <label className="flex flex-col gap-1.5 text-[13px] font-medium text-slate-700">
                ...
                {errors.petId && <span className="text-xs text-error-600">{errors.petId.message}</span>}
              </label>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="flex flex-col gap-1.5 text-[13px] font-medium text-slate-700">
                ...
                <select
                  className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20"
                  disabled={!canCheckAvailability || availabilityQuery.isLoading || availabilityQuery.isError}
                  {...register("roomTypeId")}
                >
                  <option value="">
                  ...
                  {availabilityQuery.data?.map((item) => (
                    <option key={item.roomTypeId} value={item.roomTypeId} disabled={item.availableRooms === 0}>
                      {item.roomTypeName} - {formatCurrency(item.baseDailyPriceVnd)}/ngày - còn {item.availableRooms}
                    </option>
                  ))}
                </select>
                {errors.roomTypeId && <span className="text-xs text-error-600">{errors.roomTypeId.message}</span>}
              </label>
            </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm">
                {availabilityQuery.isLoading ? (
                  <span className="inline-flex items-center gap-2 text-slate-500">
                    <Loader2 className="h-4 w-4 animate-spin" /> Đang kiểm tra phòng trống
                  </span>
                ) : availabilityQuery.isError ? (
                  <span className="text-error-600">Không thể kiểm tra phòng trống</span>
                ) : (
                  <span className="text-slate-600">
                    Chọn loại phòng để xem giá tạm tính. Hệ thống tính tối thiểu 1 ngày và làm tròn lên theo chu kỳ 24 giờ.
                  </span>
                )}
              </div>
            )}
            ...
            <Button type="submit" disabled={createBookingMutation.isPending || !selectedAvailability || selectedAvailability.availableRooms === 0}>
              <span className="inline-flex items-center gap-2">
                {createBookingMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <CalendarClock className="h-4 w-4" />}
      ...
      <Card title="Tạm tính chi phí">
        <div className="space-y-3">
          ...
          <SummaryRow label="Giá phòng/ngày" value={formatCurrency(selectedAvailability?.baseDailyPriceVnd)} />
          <SummaryRow label="Tổng tạm tính" value={<span className="text-emerald-700">{formatCurrency(estimatedPrice)}</span>} />
          <div className="rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-900">
            <CheckCircle2 className="mb-2 h-5 w-5" />
            Hóa đơn chính thức sẽ được tạo khi nhân viên check-out và xác nhận thời gian lưu trú thực tế.
          </div>
        </div>
      </Card>
    </div>
