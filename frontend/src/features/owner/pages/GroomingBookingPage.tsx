...
export function GroomingBookingPage() {
  ...
  const isPastTime = scheduledStartAt ? new Date(scheduledStartAt).getTime() <= Date.now() : false;
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
      ...
      >
        {petsQuery.isLoading || servicesQuery.isLoading ? (
          ...
          <EmptyState title="Không thể tải dữ liệu đặt lịch" description="Vui lòng thử lại sau." />
        ) : petOptions.length === 0 ? (
          <EmptyState title="Chưa có thú cưng" description="Hãy tạo hồ sơ thú cưng trước khi đặt lịch làm đẹp." />
        ) : servicesQuery.data?.length === 0 ? (
          <EmptyState title="Chưa có dịch vụ làm đẹp" description="Vui lòng liên hệ trung tâm để được hỗ trợ." />
        ) : (
          <form className="space-y-5" onSubmit={handleSubmit((values) => createBookingMutation.mutate(values))}>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="flex flex-col gap-1.5 text-[13px] font-medium text-slate-700">
                ...
                {errors.serviceId && <span className="text-xs text-error-600">{errors.serviceId.message}</span>}
              </label>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Input
                type="time"
                label="Giờ hẹn"
                error={errors.scheduledTime?.message || (isPastTime ? "Thời gian hẹn phải ở tương lai" : undefined)}
            ...
            <Button type="submit" disabled={createBookingMutation.isPending || hasBlockingError}>
      ...
      <Card title="Tạm tính lịch hẹn">
        <div className="space-y-3">
          ...
          <SummaryRow label="Thời lượng" value={formatDuration(selectedService?.durationMinutes)} />
          <SummaryRow label="Kết thúc dự kiến" value={scheduledEnd || "Chưa có"} />
          <SummaryRow label="Số tiền dự kiến" value={<span className="text-emerald-700">{formatCurrency(selectedService?.basePriceVnd)}</span>} />
          <div className="rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-900">
            {selectedService ? <CheckCircle2 className="mb-2 h-5 w-5" /> : <Sparkles className="mb-2 h-5 w-5" />}
