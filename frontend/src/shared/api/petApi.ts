...
type PageEnvelope<T> = PageResponse<T> | { success: boolean; data: PageResponse<T> | { content: T[] } };
...
export const petApi = {
  getPets: async (params?: { page?: number; size?: number; isActive?: boolean }): Promise<PageResponse<PetResponse>> => {
    const response = await axiosClient.get<any, PageEnvelope<PetResponse>>('/api/v1/pets', { params });
    return unwrapPage(response);
  },

  getPetById: (id: string): Promise<PetResponse> => {
    return axiosClient.get(`/api/v1/pets/${id}`);
  },

  createPet: (data: PetRequest): Promise<PetResponse> => {
    return axiosClient.post('/api/v1/pets', data);
  },

  updatePet: (id: string, data: PetRequest): Promise<PetResponse> => {
    return axiosClient.put(`/api/v1/pets/${id}`, data);
  },
  
  deletePet: (id: string): Promise<void> => {
    return axiosClient.delete(`/api/v1/pets/${id}`);
  }
};
